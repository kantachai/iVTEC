			...
			<div class="container-fluid">
				...
				<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
					...
					<label class="checkbox-inline">
						<a style="color:yellow; font-size:16px;" href="Manual Key_manage.html" target='_blank'>				(คู่มือการใช้งาน)
						</a> 
						<a style="color:white; font-size:16px;">วันที่ต้องการ : <input type="text" id="datepicker" style="color:black" placeholder='<?= date('d-m-Y'); ?>'></a>
						<a id="gar_printnew" href="garageReportPDF.php?datepick=<?= date('Y-m-d'); ?>" target='_blank'><u style="color:yellow; font-size:16px;"><strong style="color:yellow;">พิมพ์ตามต้นฉบับ (ผจก. อารีย์) </strong></u>/ </a>&nbsp <a id="gar_printold" href="" target='_blank'><u style="color:yellow; font-size:16px;"><strong style="color:yellow;">พิมพ์ (MD)</strong></u></a>&nbsp 
						<button type="button" class="btn btn-success btn-md garReport" data-toggle="modal" data-target="#manageGarDf" id="managedef">จัดการข้อมูลต้นฉบับ</button> &nbsp 
						<button type="button" class="btn btn-danger btn-md" id="updatecarinfo">อัพเดทข้อมูลรถทั้งหมด</button> 
						<? /* &nbsp <a style="color:yellow; font-size:16px;" href="ManualGarManage.pdf" target='_blank'>				คู่มือการจัดข้อมูลรถประจำอู่)
						</a> */
						?>
					</label>
					<form class="navbar-form navbar-left" id="defaultform" role="form">
					<?php
							...
							<?php 
								...
								foreach ($rs as $row) {
									...
									if($gar_id == 9 || $gar_id == 10){
										echo '<button type="button" value="' . $opt . ','.$gar_option.'" class="btn-nav btn btn-gar' . $gar_id . '">' . $gar_name . '</button><input id="checkgar" type="hidden" value="'.$gar_id.'">';
										}else{
										echo '<button type="button" value="' . $gar_id . ','.$opt.'" class="btn-nav btn btn-gar' . $gar_id . '">' . $gar_name . '</button><input id="checkopt" type="hidden" value="'.$gar_option.'">';
									}
								}
							?>
							&nbsp
			...
			<div class="row relative">
				<div class="col-md-12 col-xs-12">
					<?php
						...
						Personal2.dbo.CarWorkType AS cw ON c.CarWorkTypeSerial = cw.CarWorkTypeSerial
						WHERE (c.CarNo NOT LIKE N'%\_') AND (c.CarNo NOT LIKE N'z%') AND (c.CarNo NOT LIKE N'P0%') AND (c.CarNo NOT LIKE N'P1%') AND (c.CarNo NOT LIKE N'D210') AND (c.CarWorkTypeSerial IN (1, 2, 3, 5, 6, 10, 11, 12)) AND (c.CarNo IS NOT NULL)
                ...
                <div>
					<ul  id="car_sum" class="nav navbar-nav">
                        <?php
							...
							WHERE (cw.CarWorkTypeSerial IN (1, 2, 3, 5, 6, 10, 11, 12)) AND (c.CarNo NOT LIKE N'%\_') AND (c.CarNo NOT LIKE N'z%') AND (c.CarNo NOT LIKE N'P0%') AND (c.CarNo NOT LIKE N'P1%') AND (c.CarNo NOT LIKE N'D210') ";
